...
### Changed
- 데이터 출처를 **푸터 바로 위에 상시 표시** — 버전 모달에만 있던 출처를 화면 하단(푸터 위)에 작게 회색으로 항상 노출(ksbsa·dbsa·gameone 링크)

## [1.3.4] - 2026-06-30

### Added
- 버전 정보 모달 상단에 **데이터 출처** 표시 — `ksbsa.or.kr`·`dbsa`·`gameone.kr` 등 출처 사이트 링크(푸터는 미니멀 2줄 유지)
